...
def actors():
    # GET
    if request.method == "GET":
        actors = db_read("SELECT a.id, a.actor_fname, a.actor_lname, a.actor_email , a.actor_size, r.role_name FROM actors a LEFT JOIN roles r ON a.role_id = r.id WHERE a.user_id=%s ORDER BY actor_fname", (current_user.id,))
...
def overview_roles():
    # GET
    if request.method == "GET":
        roles = db_read("SELECT roles.role_name, actors.actor_fname, actors.actor_lname, costumes.costume_name, costumes.costume_size FROM roles LEFT JOIN actors ON roles.id = actors.role_id LEFT JOIN costumes ON roles.id = costumes.role_id WHERE user_id=%s ORDER BY role_name"(current_user.id))
